    ...
    <div class="control-group" ng-class="{error: ${formProperty}.$invalid}">
        ...
        <div class="controls">
            <div ng-repeat="${property.name}Element in ${modelProperty}">
                <select id="${property.name}{{$index}}" name="${property.name}{{$index}}" ng-model="${modelProperty}[$index]" ng-options="${property.name?substring(0, 1)} as ${property.name?substring(0, 1)}.id for ${property.name?substring(0, 1)} in ${property.name}List">
